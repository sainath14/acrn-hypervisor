...
void get_buffer_for_ept_pages(void)
{
	...
	for (vm_id = 0U; vm_id < CONFIG_MAX_VM_NUM; vm_id++) {
		...
		size = PT_PAGE_NUM(vm_address_space_size[vm_config->load_order])*MEM_4K;
		pt_base = e820_alloc_memory(size, ~0UL);
		hv_access_memory_region_update(pt_base, size);
		ept_pages_info[vm_id].ept.nworld_pt_base = (struct page *)pt_base;
	}
}
#else
