			...
			struct ptirq_remapping_info *entry, uint32_t vector)
{
	...
	/* Using vector as index in the corresponding IOMMU */
	...
	irte.bits.dest.x2apic_id = dest_mask;
	ret = dmar_assign_irte((uint8_t)(entry->phys_sid.msi_id.bdf >> 8U),
			(uint8_t)(entry->phys_sid.msi_id.bdf & 0xffU),
			irte, vector);

	/* update physical delivery mode & vector */
	if (ret == 0) {
		info->pmsi_data.full = 0U;
		ir_index.index = vector;

		info->pmsi_addr.full = 0UL;
		info->pmsi_addr.ir_bits.intr_index_1 = ir_index.bits.index_2;
		info->pmsi_addr.ir_bits.shv = 0U;
		info->pmsi_addr.ir_bits.intr_format = 0x1U;
		info->pmsi_addr.ir_bits.intr_index_2 = ir_index.bits.index_1;
...
ptirq_build_physical_rte(struct acrn_vm *vm, struct ptirq_remapping_info *entry)
{
	...
	if (virt_sid->intx_id.src == PTDEV_VPIN_IOAPIC) {
		...
		dmar_assign_irte_ioapic(ioapic_irq_to_ioapic_id(phys_irq), irte, vector);


		ir_index.index = vector;
		rte.ir_bits.vector = vector;
		rte.ir_bits.constant = 0U;
		rte.ir_bits.intr_index_1 = ir_index.bits.index_2;
		rte.ir_bits.intr_format = 1U;
		rte.ir_bits.intr_index_2 = ir_index.bits.index_1;
...
remove_msix_remapping(const struct acrn_vm *vm, uint16_t virt_bdf, uint32_t entry_nr)
{
	...
	if (entry != NULL) {
		...
		dmar_free_irte((uint8_t)(entry->phys_sid.msi_id.bdf >> 8U),
				(uint8_t)(entry->phys_sid.msi_id.bdf & 0xffU),
	...
	if (((!pic_pin) && (virt_pin >= vioapic_pincount(vm))) || (pic_pin && (virt_pin >= vpic_pincount()))) {
		...
		if (entry != NULL) {
			if (is_entry_active(entry)) {
				...
				dmar_free_irte_ioapic(ioapic_irq_to_ioapic_id(phys_irq), irq_to_vector(entry->allocated_pirq));
				dev_dbg(ACRN_DBG_IRQ,
