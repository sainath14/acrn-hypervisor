...
#define IOAPIC_INVALID_ID	0xffffU
...
uint16_t ioapic_irq_to_ioapic_id(uint32_t irq)
{
	uint16_t ret;
...
bool ioapic_is_ioapic_id_valid(uint16_t ioapic_id)
{
	return (ioapic_id != IOAPIC_INVALID_ID);
}
...
{
	uint8_t ioapic_id;
	struct ioapic_info ioapic_local_array[CONFIG_MAX_IOAPIC_NUM];

	ioapic_num = parse_madt_ioapic(ioapic_local_array);

	for(ioapic_id = 0;
		ioapic_id < ioapic_num; ioapic_id++) {
		ioapic_array[ioapic_id].id = ioapic_local_array[ioapic_id].id;
		ioapic_array[ioapic_id].addr = ioapic_local_array[ioapic_id].addr;
		ioapic_array[ioapic_id].gsi_base = ioapic_local_array[ioapic_id].gsi_base;
	}
}
...
void ioapic_setup_irqs(void)
{
	...
	for (ioapic_id = 0U;
	     ioapic_id < CONFIG_MAX_IOAPIC_NUM; ioapic_id++) {
		void *addr;
...
void suspend_ioapic(void)
{
	...
	for (ioapic_id = 0U; ioapic_id < CONFIG_MAX_IOAPIC_NUM; ioapic_id++) {
		void *addr;
		uint32_t nr_pins;
