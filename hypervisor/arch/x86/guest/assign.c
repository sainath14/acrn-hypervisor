		...
		if (is_entry_active(entry) && (info->vmsi_data.full == 0U)) {
			...
			if (is_lapic_pt_configured(vm)) {
				...
				if (vlapic_state == VM_VLAPIC_X2APIC) {
					 ...
					 * as device is not programmed with interrupt info. ACRN can use
					 * Interrupt Remapping Table to mask the interrupts during transition
					 * and unmask them once transition is done.
					 */
					ret = -EFAULT;
				}
			} else {
